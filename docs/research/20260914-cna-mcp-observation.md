...
**`cna-news-latest-newslist`** — 看中央社編輯選的「今日焦點/重點新聞」,不需參數。_(未實測)_

**`cna-news-latest-category`** — 看某分類的最新新聞,分類為 enum 值(政治、國際、社會、產經、科技等)。_(未實測)_

**`cna-news-single`** — 用 `pid` 或完整標題取單篇新聞全文,設計上與 `cna-news-qsearch` 互補(先搜再取全文)。_(未實測)_
...
**`cna-word-map`** — 單一關鍵字延展工具,用於搜尋 0 筆時校正或延伸查詢詞。_(未實測)_

**`cna-translation-lookup`** — 查中央社慣用的外國人名/地名/組織譯名。_(未實測)_

**`cna-photo-search`** — 搜中央社影像空間圖片,僅回傳已上架(SHOW=true)的圖片。_(未實測)_
...
_本報告基於實際對話中的工具呼叫測試整理而成,涵蓋範圍限於本次對話中實際呼叫或查詢到的工具與參數;未標明「已實測」的項目均為文件描述,未經呼叫驗證。_
